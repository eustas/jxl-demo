...
(() => {
  ...
  const EMBEDDED = {
    'client_worker.js': 'let decoder=null,workQueue=[];const processQueue=()=>{for(;workQueue.length;){var e=workQueue.shift(),e=(console.log("CW job: "+e.uid),{uid:e.uid,result:"bad"});console.log("CW->Client: "+JSON.stringify(e)),postMessage(e)}},onLoadJxlModule=(onmessage=function(e){var o=e.data;console.log("CW received: "+o.op),"decodeJxl"===o.op&&(workQueue.push(e.data),processQueue())},e=>{decoder=e,processQueue()}),config=(importScripts("jxl_decoder.js"),{mainScriptUrlOrBlob:"https://jxl-demo.netlify.app/jxl_decoder.js"});JxlCodecModule(config).then(onLoadJxlModule);',
